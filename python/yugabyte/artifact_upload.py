        ...
        while True:
            ...
            try:
                if method == UploadMethod.SSH:
                    ...
                    subprocess.check_call(['ssh', '-o', 'StrictHostKeyChecking=no', dest_host, 'mkdir', '-p', dest_dir])
                    subprocess.check_call(['scp', '-o', 'StrictHostKeyChecking=no', artifact_path, f'{dest_host}:{dest_dir}/'])
                elif method == UploadMethod.CP:
